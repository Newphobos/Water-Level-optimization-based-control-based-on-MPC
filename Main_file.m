...
%->prediction horizon
U = SX.sym('U',n_controls,Np + 1 ); % Control inputs matrix
H = SX.sym('H',n_states,(Np + 1)); % States matrix
 
%->.... Parameter vector to store initial states and disturbance
P = SX.sym('P',n_states + Np,1); % 2 initial states and Np number of 
...
OPT_variables = [reshape(H,2*(Np+1),1);reshape(U,2*(Np+1),1)];
...
% -0.01 <= delta_u <=0.01
args.lbg = repelem([0;-0.01],[2*(Np+1),2*Np]);
args.ubg = repelem([0;0.01],[2*(Np+1),2*Np]);

%->....Bounds on gate opening, U
args.lbx(2*(Np+1)+1:1:2*(Np+1)+ 2*(Np+1),1) = 0; % Lower limit of gate opening
args.ubx(2*(Np+1)+1:1:2*(Np+1)+ 2*(Np+1),1) = hg_max;
...
initial_control = [0;0];
U0 = repmat(initial_control,1,Np+1)';
...
for i=1:timesteps
    ...
    args.x0 = [reshape(H0',2*(Np+1),1);reshape(U0',2*(Np+1),1)];
...
%     %->....Bounds on h2
    ...
    u = reshape(full(sol.x(2*(Np+1)+1:end))',2,Np+1)';
...
plotting_results(x,u_eachstep,Vgate_each,Vgate_total,generate_disturbance,...
                  xLRV,xHRV,timesteps,time_eachloop,Average_looprun_time)
